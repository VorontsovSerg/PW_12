...
public class MainActivity extends AppCompatActivity {
    ...
    private static final String[] MOVIES = {
            "Побег из Шоушенка", "Крестный отец", "Темный рыцарь",
            "Форрест Гамп", "Начало"
    };
    ...
    private void fillMoviesTable() {
        ...
        for (int i = 0; i < MOVIES.length; i++) {
            ContentValues values = new ContentValues();
            values.put("title", MOVIES[i]);

            // Вставка данных через Content Resolver
            try {
                Uri insertedUri = getContentResolver().insert(CONTENT_URI, values);
                if (insertedUri == null) {
                    Log.e("FillMoviesTable", "Ошибка при вставке фильма \"" + MOVIES[i] + "\" в таблицу");
                }
            } catch (Exception e) {
                e.printStackTrace();
                Log.e("FillMoviesTable", "Ошибка при вставке фильма \"" + MOVIES[i] + "\" в таблицу: " + e.getMessage());
            }
        }
    }
}
